...
function use(items) {
  ...
  if (usingWysiwygEditor()) {
    ...
    // window.open(url,"_blank");
      const selectedItems = getSelectedItems(); // Получаем выбранные файлы
      if (selectedItems.length > 0) {
          const fileUrl = selectedItems[0].url; // Берём URL первого файла
          window.opener.postMessage({ fileUrl: fileUrl }, "*"); // Отправляем в родительское окно
          window.close(); // Закрываем файловый менеджер
      }
  } else {
